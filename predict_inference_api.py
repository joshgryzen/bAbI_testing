...
# from handle_http_429_errors import retry_if_http_429_error, wait_for_retry_after_header
...
#     wait=wait_for_retry_after_header(fallback=wait_fixed(360)),
#     stop=stop_after_attempt(3),
# )
@retry(
    wait=wait_fixed(3610),
)
def get_pred(prompt, model):
    return (
        inference.text_generation(prompt, model=model, max_new_tokens=2)
        .strip()
        .replace(".", "")
    )
